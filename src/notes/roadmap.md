...
> the ladder you'll have _built_, line by line, every component that Phase 0's vertical slice assembles —
> and you'll understand not just _what_ the code does but _what's happening underneath_ and _how to prove
> it's correct_.
...
- Keep each module as its own throwaway script under a scratch folder (e.g. `experiments/`), _not_ in
...
A good test answers: _"if this function were silently wrong, what input would expose it?"_ Three
...
- **Failure case** — bad input _should_ fail loudly (raise/return an error), not silently produce garbage.
...
`sys.executable`.
**You learn:** That `sys.executable` points _inside_ `.venv\` — proof you're running the isolated
...
Make the top-left pixel pure blue _in BGR_ (`frame[0,0] = [255,0,0]`). Print `frame.shape`, `frame.dtype`,
`frame.nbytes`. Slice out a 2×2 crop. Swap channel 0 and channel 2 (BGR→RGB) two ways: `frame[..., ::-1]`
and `cv2.cvtColor`.
**You learn:** A frame _is_ an `(H, W, 3) uint8` ndarray — a flat byte buffer with shape metadata. Cropping
is just slicing (a _view_, not a copy — mutate it and you mutate the original). Why `nbytes = H*W*3`. The
BGR convention, concretely.
**Test cases:** Assert `shape == (4,6,3)` and `dtype == uint8`. Assert the hand-swapped array equals the
`cvtColor` result (`np.array_equal`). Assert that mutating a slice _without_ `.copy()` changes the parent
...
`cv2.imwrite`. Then load the _saved_ copy and compare.
...
is **lossy** — the reloaded array is _not_ byte-identical.
**Test cases:** Assert `imread` of a real file returns an ndarray; assert `imread` of a nonexistent path
returns `None` (and write an explicit guard that raises a clear error). Assert reloaded JPEG is _close to_
but _not equal to_ the original (`np.array_equal` is False; mean abs diff is small).
**Debugging drill:** Pass a path with a typo and watch the `None` propagate into a cryptic crash three lines
later — then add the guard that turns it into a clear message at the source. (Failing _early and loudly_ is
...
smooths away the fine stitching that Pillar 1 needs — _more preprocessing is not always better_.
**Test cases:** Assert resized shape is exactly `(1024,1024,3)`. Assert denoised image has lower pixel
variance than the noisy original (denoise _reduces_ high-frequency content). Assert threshold output
...
read `cv2.resize`'s `(width, height)` ordering, which is the _opposite_ of NumPy's `(rows, cols)`. This
...
junk contours — learn that the _preprocessing_ (M5) is what makes the _detection_ (M6) work. Garbage in,
...
re-encoding). _Why_ each step exists: compression for bandwidth/latency, base64 because JSON is a text
...
`UnicodeDecodeError` / corruption. That error _means_ "you put binary in a text channel."
...
Why you hash the _preprocessed canonical_ bytes, not the raw frame (raw frames are never byte-identical
...
the key change every time → 0% cache hit rate. This _is_ the bug that motivates canonicalization before
hashing.
...
**Test cases:** Hard to unit-test live hardware, so test the _seams_: assert `cap.isOpened()` is True after
...
**Test cases:** Feed a _saved still_ (not the live camera) through the preprocess+hash chain so it's
...
saved file — trace _which_ stage introduced it by saving intermediate outputs. Saving intermediates is a
core CV debugging technique: you can't `print` an image, so you _look_ at it.
...
response. Where latency lives (network + model inference). Secrets come from the _environment_, never code.
What failures look like (auth error with a bad key, timeout, rate-limit 429).
**Test cases:** Assert the key loads and is non-empty _before_ calling (fail fast with a clear message if
`.env` is missing). Wrap the call so a network error returns a typed result you can assert on, rather than
crashing. (You're testing _your_ error handling, not Google's API.)
**Debugging drill:** Run with a deliberately wrong API key and read the auth error; run with a 1ms timeout
and read the timeout error. Knowing what each failure _looks like_ is half of debugging integrations.
...
**Build:** Capture/load a frame → BGR→RGB → JPEG-encode (M7) → send to Gemini with _"Reply with only YES or
NO: is there a sneaker in this image?"_ → parse and print. Test on a shoe and on a non-shoe.
...
it raise, default to False, or return `UNKNOWN`? Designing for the _ambiguous_ answer is exactly the
...
**You learn:** Read-through caching and TTL semantics _before_ Redis exists — Phase 5 just swaps the dict
for Redis with the _same interface_. How to structure a `pytest` suite: arrange/act/assert, fixtures for
...
_Companion to `architecture.md`. Do the rungs in order; write the tests; break things on purpose._
